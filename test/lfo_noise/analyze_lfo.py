...
README は `[A]`〜`[R]` の通し番号で集計を指すが、**`[H]` `[I]` はこのスクリプトの外側**
（`opm-lfo-period.py` を直接呼んで作った TSV と `--gaps`）で、**`[K]` `[L]` `[M]` `[O]` は
README 側だけの番号**（いずれも `--values` の出力で、この節はラベルを持たない）。
対応は README.md §4.5 の `result/` の表で引く。
