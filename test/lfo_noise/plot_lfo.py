...
`analyze_lfo.align_values()` ではなく `grid_edge()` で生波形から直接決めている。
理由はそちらの docstring を参照。

## 1 枚のパネルに描いてあるもの

- **灰色の細線** — キャプチャした生のサンプルそのもの（振幅は線形）。搬送波の
  正弦波が LFO の段ごとにレベルを変えているのが直接見える
- **青い階段** — 段ごとの搬送波レベル [dB]。`analyze_lfo.block_values()` の出力
- **背景の縞** — 段 1 つぶん。`2^(18 - (LFRQ >> 4))` サンプル
- **細い縦線** — 8 サンプルごと（LFO の最短ラッチ間隔）

## 3 枚で揃えてあるもの

図をまたいで比べられるように次を固定してある。**別の図からパネルを切り出して
並べても縮尺が合う。**

- 横軸: 256 サンプル（62500Hz で 4.096ms）。1 サンプル = 3px
- 縦軸: 搬送波レベル +2〜-50dB。パネルの寸法と余白
- 生波形の振幅の基準（そのキャプチャの |x| の上位 0.1% 点をパネル半分の高さに取る）

## 縦軸について

AM 測定（`AMD=0x7f` / `AMS=2`）では搬送波の振幅が LFO 値に対して dB で線形に下がる。
**レベル [dB] は LFO 値の定数倍**なので、階段の高さがそのまま LFO 値（を上下反転
したもの）になる。絶対値は取れないので 0dB 基準はキャプチャごとに取り直している
（段のレベルの上位 0.2% 点 = LFO 値 0 の段）。段の幅と段差の比較にはこれで足りる。

生波形は線形の振幅なので、レベルが 20dB 下がった段は 1/10 の高さにしか描かれない。
**段の幅と位置を読むのは青い階段のほう**で、生波形はそれが実測であることと、
...
生成時に次を表示する。窓に入った段数が予測とずれていれば `NG` を出して終了コード 1。

- 窓内の段数 / `2^(18 - (LFRQ >> 4))` から出した予測段数
- キャプチャ全体で値が変わった段の割合 `c`（`analyze_lfo.changed_fraction()`）
- 隣の段とレベルがほぼ同じだった境界の割合（図 3 の ▼ の割合）
"""
