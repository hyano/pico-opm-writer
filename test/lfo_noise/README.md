...
| パネル | 波形の出どころ |
| --- | --- |
| `LFRQ=0xb0` | `wav/am_nfrq_1f_lfrq_b0_kc_4a_mul_04.wav.zst` |
...
横軸の縮尺・縦軸のレンジ・パネルの寸法は 3 枚の図で共通。図の生成は [§4.5 図の生成](#図の生成)。
...
| パネル | 波形の出どころ | キャプチャ全体で ▼ が付く境界の割合 |
| --- | --- | --- |
| `LFRQ=0xf0` / `NFRQ=0x1f` | `wav/am_nfrq_1f_lfrq_f0_kc_52_mul_08.wav.zst` | 2.2% |
| `LFRQ=0xf0` / `NFRQ=0x00` | `wav/am_nfrq_00_lfrq_f0_kc_52_mul_08.wav.zst` | **44.8%** |
| `LFRQ=0xe0` / `NFRQ=0x00` | `wav/am_nfrq_00_lfrq_e0_kc_52_mul_04.wav.zst` | 1.9% |

割合は `./plot_lfo.py` が表示する（0.5dB という境目は、同じ語を続けて引いた段の差が
0.21dB（中段の偶数境界の中央値）に集まるのに対し、別の語どうしの差は中央値 14dB あることから
決めた）。理想の 50% に届かないぶんは 8 サンプルという短い段での測定誤差。
同じことを閾値なしの統計で測ったのが
[§5.4.2](#542-段ごとの-lfo-値から供給周期を測る) で、こちらは奇数の連が 1 本も出ないことを示している。図の生成は [§4.5 図の生成](#図の生成)。
