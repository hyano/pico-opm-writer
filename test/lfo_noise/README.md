...
- 位置は **`lo <= 7` では全部が奇数、`lo >= 8` では全部が偶数**に揃う

`lo = 7` と `lo = 8` の境でパリティが入れ替わるので、`LFRQ` で言えば **`0x27` と `0x28`
のあいだ**。[§5.5.1](#551-値のレベルで見ると帯全体で-1-段おきにしか変わっていない) の実測はまさにそこで高い側が入れ替わっている。
16 値すべてで突き合わせた結果は [§5.5.6](#556-下位ニブル-16-値で山とパリティの反転位置を突き合わせる)。
