...
`wav_nfrq/` と `wav_nfrq_5a09/` で全 32 値を押さえてある**（[§4.4](#44-データセット) / [§5.4](#54-nfrq-を全-32-値振る)）。
...
| **その瞬間に値が実際に変わるかどうか** | 主にノイズ語の更新周期 `P_n`（[§2.2](#22-lfo-値が変わる間隔はノイズ語の更新周期との合成で決まる)）。`LFRQ` の下位ニブルも関与していそうだが確定していない（[§5.5](#55-lfrq0x28-の異常を段数-10-倍でキャプチャし直す) / [§7](#7-不明点未解決事項)） |
...
（[§4.5](#45-解析と集計の手順) / [§4.6](#46-段ごとの-lfo-値を取り出す解析)）。集計は主張ごとに 1 節（`[A]`〜`[J]`）に分かれている。
...
もし LFO がその瞬間に語を取り込んでいれば、**別の値になるはず**である。それが
1 段おきに起きないということは、**その回は取り込み自体が起きていない**と考えられる。
「同じ語を 2 回引いた」のではない（ノイズ発生器の系列がちょうど整数周回する場合を除く）。
...
- **`NFRQ=0x1f` / `LFRQ` 下位ニブル `8` 付近で更新が飛ぶ仕組み**（[§5.5](#55-lfrq0x28-の異常を段数-10-倍でキャプチャし直す)）。
  ...
  かつて残していた説明は「ノイズ発生器の走る周期と LFO の更新周期が整数比で噛み合い、
  同じ値が続けて読まれている」というものだったが、**`P_n` が決まった今これは棄却できる**。
  `LFRQ=0x28` の `P_lfo` は 65536 サンプルで、`P_n` は `NFRQ=00` でも 16、`1f` なら 0.5。
  ノイズの方が 4096〜131072 倍速いので、[§2.2](#22-lfo-値が変わる間隔はノイズ語の更新周期との合成で決まる) の「同じ語を引く」機構はこの帯では
  起こりようがない。**別の仕組みが要る。**

  値のレベルで見ると、これは帯 `0x24`〜`0x2c` 全体で起きていて、`0x27`/`0x28` で最も強い
  （[§5.5](#55-lfrq0x28-の異常を段数-10-倍でキャプチャし直す)）。**取り込み自体が 1 段おきに飛んでいる**ところまでは言えるが、
  なぜ飛ぶのかは分からない。

  「1 ラッチあたりにノイズ発生器が進む量が系列の周期と噛み合い、同じ語を 2 回引いている」
  という説明は成り立たない。`NFRQ=0x1f` でこの帯が 1 ラッチで進む量は系列 1 周
  （[test/noise_period/](../noise_period/README.md)）の整数倍から外れるので、取り込んでいれば
  必ず別の語になる。噛み合わせで説明するなら、進み量が周期の整数倍になる上位ニブルで
  同じことが起きるはずだが、そういう帯は無い。
...
- **値列の突き合わせの「上限」を測る**: 実行済み。同じ下位ニブルを 3 セッション撮った組を
  ...
  **残る課題は、キャプチャどうしの位相ズレに左右されない測り方を用意すること。** 候補は
  ...
  頼らずに比較する方法。
- **`0x28` 付近で更新が飛ぶ仕組み**: `NFRQ` を振って、飛び方が `NFRQ` に依存するかを見る。
  分かっているのは `NFRQ=0x1f` で起きて `0x00` では起きないことだけで、あいだの 30 値は
  測っていない。**`0x1f` だけで起きるのか、ある値から連続的に強くなるのかで、
  ノイズ発生器側の事情か LFO 側の事情かが分かれる。**

  帯 9 値 × `NFRQ` 31 値 × 2 モードを素直に振ると 120 秒 × 558 本 = 約 20 時間になるので
  **2 段に分ける**。まず **`LFRQ=0x28` の 1 点で `NFRQ` 全 32 値**（64 本、約 2.5 時間）を取り、
  次に**応答が出た `NFRQ` に絞って**帯 `0x24`〜`0x2c` に広げる。

  ```bash
  cd test/lfo_noise
  NF=$(python3 -c "print(','.join('%02x' % i for i in range(32)))")

  # 第 1 段: LFRQ=0x28 の 1 点 × NFRQ 全 32 値 × AM/PM = 64 本（約 2.5 時間）
  ...
  判定は周期推定ではなく [§5.5](#55-lfrq0x28-の異常を段数-10-倍でキャプチャし直す) の**段境界の変化率**で行う。しきい値を通さず
  0〜100% の連続量として出るので、山の形をそのまま追える。`NFRQ=0x00` は対照
  （両パリティが同程度になるはず）で、`0x1f` は既知（片方が 0〜36%）。
  **この 2 点に挟まれた 30 値がどう動くか**が読みどころ。
