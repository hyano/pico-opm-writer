...
| `wav_w0/` `wav_w1/` `wav_w2/` | 256 × 3 | 自動選択 | W=0,1,2 の波形（**一次データのみ**、[§5.7](#57-一次データだけキャプチャしてあるもの)） |
...
**中間の下位ニブルの 30〜43% は、一部の段だけが違う値になっていることを意味する。**
一致率を 256 段ごとの窓で出しても一様（`0xa4` で 39〜47%、`0xa1` で 37〜42%）なので、
「途中から系列がずれていく」のではなく、**常に一定割合の段だけが違う**。
`0xaf` は同じ窓で全区間 100%。

ただし**この 30〜43% という値そのものは深追いできない。** 同一条件どうしでも 73% に
落ちる組があるので、この測り方の上限が常に 100% とは限らないためである
（[§7](#7-不明点未解決事項) / [§8](#8-今後の課題)）。

**「下位ニブルが大きいほど値列が食い違う」という描像は成り立たない。** それなら `0xaf` が
最もよく食い違うはずのところ、実測はその逆で `0xaf` だけが完全一致する（[§7](#7-不明点未解決事項)）。

### 5.7 一次データだけキャプチャしてあるもの

次の 1 群は**キャプチャと長さの検査までで、解析はしていない**。今の解析器
...
- **中間の下位ニブルで値列の一致率が 30〜43% に留まるのはなぜか。** `0` と `f` は
  同じ値列を出す（[§2.3](#23-下位ニブルは値列も変えない)）のに、`1` `2` `4` `8` は一定割合の段だけが違う値になる
  （[§5.6](#56-段ごとの値列を突き合わせる)）。**下位ニブルの大きさに応じて値列が連続的に変わる**という描像なら
  `f` が最もよく食い違うはずだが、実測はその逆で `f` だけが完全一致する。説明が付いていない。
  **この 30〜43% を額面どおり受け取れないことも問題**で、同一条件どうしの組でも 73% に
  落ちる例があり、この測り方の上限が常に 100% とは限らない。切り分けの手順は [§8](#8-今後の課題)。
- **他の LFO 波形（W=0,1,2）で [§2.1](#21-更新周期は-lfrq-の上位ニブルだけで決まる) の式が成り立つか。** データは
  `wav_w0/` `wav_w1/` `wav_w2/`（[§5.7](#57-一次データだけキャプチャしてあるもの)）。W≠3 では LFO 値が周期波形になるので、
...
- **値列の突き合わせの「上限」を測る**: [§5.6](#56-段ごとの値列を突き合わせる) の中間の下位ニブルの 30〜43% が
  下位ニブルの効果なのか測り方の限界なのかを、いまのデータでは切り分けられない。
  同一条件どうしでも 73% に落ちる組があるためである。
  **切り分けるには、同じ下位ニブルを何本も撮って一致率の分布を出す。**
  一次データは `wav_value/` `_r2` `_r3` に全 16 値ぶんあるので**実機は要らない**。
  ここが常に 100% なら 30〜43% は下位ニブルの効果、73% 前後まで散るなら測り方の限界。
  ...
  基準（1 本目）を 3 つのキャプチャで取り替えて全組を出す。同じファイルどうしの組は
  必ず 100% になるので、それが目印になる。
  足りなければ `0xa0` と `0xa4` を 8 本ずつ撮り直す（`--steps 2048 --min-ms 8400` で
  1 本 8.4 秒なので 16 本でも 5 分）。**揃う組の割合そのもの**も、ノイズ発生器の位相が
  キャプチャごとにどれだけ揺れるかの測定になる。
...
- **W=0,1,2 で [§2.1](#21-更新周期は-lfrq-の上位ニブルだけで決まる) の式が成り立つか**: `wav_w0/` `wav_w1/` `wav_w2/` はキャプチャ済み
  ...
  **更新間隔ではなく波形そのものの周期を測る解析**が別に要る。W=3 で下位ニブルが
  無視されるのがノイズ発生器側の事情なら、W≠3 では [§4.1](#41-検証対象の想定モデル) の想定モデルどおり
  下位ニブルが効くはずで、そこが分かれ目になる。
