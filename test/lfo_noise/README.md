...
| `LFRQ` 上位ニブルと「値が 2 段ずつ対になる」現象の関係 | 上位ニブル `2` でだけ起きる。同じ下位ニブル `8` の `0x18`（hi=1）/ `0x38`（hi=3）では起きない | [§5.5.5](#555-hi2-の帯を-nfrq0x1f-で当てると対にならない)（推定周期は 8 本とも規則どおり、段境界の変化率も 8 マス平坦） |
...
| `LFRQ` 上位ニブル | **`2` だけ**。同じ下位ニブル `8` の `0x18`（hi=1）と `0x38`（hi=3）を `NFRQ=0x1f` で当てても、周期は 2 倍にならずパリティも割れない（[§5.5.5](#555-hi2-の帯を-nfrq0x1f-で当てると対にならない)） |
...
| 2 段ずつ対になるのは上位ニブル `2` だけ | `0x18` / `0x38` を `NFRQ=0x1f` で当てた対照 | 推定周期は 8 本とも規則どおり（2 倍にならない）。段境界の変化率も 8 マスすべて平坦 | [§5.5.5](#555-hi2-の帯を-nfrq0x1f-で当てると対にならない) |
...
| `wav_hi_nfrq1f/` | 8 | 自動選択（`4a/4`） | `hi≠2`（`0x18` / `0x38`）を `NFRQ=0x1f` で当てた対照（[§5.5.5](#555-hi2-の帯を-nfrq0x1f-で当てると対にならない)） |
...
| `wav_w0/` `wav_w2/` | 256 × 2 | 自動選択 | W=0,2 で下位ニブルが更新周期に効くか（[§5.7](#57-w012-でも更新間隔は上位ニブルだけで決まる)）と歩幅（[§5.8](#58-鋸-w0-の-1-周期の段数から歩幅を測る)） |
...
| `summary_wav_hi_nfrq1f_values.txt` | `hi≠2` を `NFRQ=0x1f` で当てた対照（[§5.5.5](#555-hi2-の帯を-nfrq0x1f-で当てると対にならない)） |
...
W=0（鋸）と W=2（三角）の 2 波形で裏付けが取れている（[§5.7](#57-w012-でも更新間隔は上位ニブルだけで決まる)）。
...
#### 5.5.5 `hi≠2` の帯を `NFRQ=0x1f` で当てると対にならない
...
根拠は `result/summary_wav_w0_cycle.txt`（`[R]`）。[§5.7](#57-w012-でも更新間隔は上位ニブルだけで決まる) で
...
`wav_w0/` の AM 全 128 条件（上位ニブル `8`〜`f` × 下位ニブル 16 値）の結果:

| 上位ニブル `8` の帯 | 1 周期 [samples] | 予測 `2^30/((16+lo)·2^hi)` | 段数 | 予測 `4096/(16+lo)` | 歩幅 | 予測 `(16+lo)/16` |
| --- | --- | --- | --- | --- | --- | --- |
...
- **128 条件すべてで段数が予測と 1 段以内で一致する**（最大のずれ 0.95 段）。
  折り返しの位置がブロック単位でしか決まらないぶんのぶれが最大 ±4 段あるので、
  そのなかに収まっている
...
| | 組数 | 一致率 | 遅れ |
| --- | --- | --- | --- |
| 閾値 (90%) 以上 | 379 | **97.3〜100.0%** | -13〜8 段 |
| 閾値未満 | 749 | **56.8〜61.1%** | - |

あいだの 61.1〜97.3% には 1 組も無い。偶然の上限（外れた遅れでの一致率の上側 99% 点）は
最大 56.3% なので、下の塊はそのまま偶然の水準にある。**閾値をこの空白のどこに置いても
結果は変わらない。**
...
| --- | --- | --- |
| 1 フレーム | `0x1f` だけ | **一致**（`0x1f` だけが立ち、`0x00`〜`0x1e` の 31 値は平坦） |
| 2 フレーム | `0x1e` でも立つ（`m` = 1）| **不一致**。`0x1e` は AM 95/96・PM 93/91 で隣の `0x1f` との間に段差がそのまま立つ |
| 4 フレーム | `0x1c` でも立つ | **不一致**。同様に平坦 |

`NFRQ` を全 32 値振ってあるので、**Δ が 2 以上なら `0x1f` より手前にもう 1 本
山が立っていなければならない。それが 1 本も無い。** よって Δ = 1 フレーム。
...
3 つ目は [§5.5.4](#554-山の大きさを追加の取り込みの密度で説明する) が「説明できていない」としていたもので、
**そこで仮定していた `lo/16` の桁上げパターンが誤っていた**だけだった。上のデコード表で
「追加あり」の直後に「追加なし」が来る位置を数えると、`lo <= 7` では全部が奇数、
`lo >= 8` では全部が偶数になり、割合はちょうど `min(lo, 16-lo)/16` になる
（`analyze_lfo.py --self-test` が 16 値を直接検算している）。
...
- **1 フレームより細かい位置**: DAC 出力からは原理的に届かない。φ1 を直接観測できる
  測定系（ロジアナを φ1 に同期させ、SO の各ビットを追う）に変えるか、内部信号を
  読めるエミュレータで代用するしかない。**本調査の測定系では扱わない。**
