...
#### 値のレベルで見ると、帯全体で 1 段おきにしか変わっていない
...
ため、これだけでは区別できない（下記の測り直しで判明）。

**「下位ニブルが大きいほど値列が食い違う」という描像は成り立たない。** それなら `0xaf` が
最もよく食い違うはずのところ、実測はその逆で `0xaf` だけが完全一致する。

#### この測り方の「上限」を同一条件の繰り返しで測る

根拠は `result/summary_wav_value_repeat.txt`（[§8](#8-今後の課題) に書いたコマンドで生成、
実機は使っていない）。上の 30〜43% が下位ニブルの効果なのか測り方の限界なのかを、
...
- **`1` `2` `4` `8` で LFO の値列が本当に一致するのかしないのかが分からない。** `0` と `f` は
  同じ値列を出す（[§2.3](#23-下位ニブルは値列も変えない)）が、中間の下位ニブルの真の一致は 30〜43% に
  留まる。同じ下位ニブルを 3 セッション撮った組どうしで測り直すと、真の一致は
  20%〜100%（`0xa8` は 6 組中 1 組も一致しない）まで散り、**条件を揃えてもこの範囲まで
  落ちる**ことが分かった（[§5.6](#56-段ごとの値列を突き合わせる)）。つまり中間ニブルの 30〜43% は
  「値列が実際に一部食い違っている」ことと「キャプチャどうしの位相がたまたま合わなかった」
  ことのどちらでも説明が付き、**2 本の独立なキャプチャを位相だけ合わせて突き合わせる今の
  測り方では区別できない。** `f` が最もよく食い違うはずのところ実測はその逆という点も
  未説明のまま残る。位相ズレに左右されない測り方が無いと切り分けられない。
- **下位ニブルが変える「歩幅」の値。** W=0 / W=2 で更新間隔が動かないこと自体は
  ...
  というところまでしか見えていない。測り方は [§8](#8-今後の課題)。
...
- **値列の突き合わせの「上限」を測る**: 実行済み。同じ下位ニブルを 3 セッション撮った組を
  総当たりで突き合わせた結果は [§5.6](#56-段ごとの値列を突き合わせる)（根拠 `result/summary_wav_value_repeat.txt`、
  以下のコマンドで生成、実機は使っていない）。一致した組の真の一致は下位ニブルによって
  20%〜100% に散らばり、`0xa8` は 6 組中 1 組も一致しなかった。**同一条件でもこの範囲まで
  落ちる以上、中間ニブルの 30〜43% は測り方の限界であって下位ニブルの効果とは言えない**
  と判断した。
  ...
  **残る課題は、キャプチャどうしの位相ズレに左右されない測り方を用意すること。** 候補は
  [test/noise_period/](../noise_period/README.md) のようにノイズ発生器のビット列を直接読む
  手法と LFO 経由の値列を同一キャプチャ内で結び付け、2 本の独立なキャプチャの位相合わせに
  頼らずに比較する方法。
- **`0x28` 付近で値が対になる仕組み**: 実行済み。`LFRQ=0x28` の 1 点で `NFRQ` 全 32 値を
  撮り、段境界の変化率で判定した結果は [§5.5.3](#553-nfrq-を全-32-値振ると-0x1f-だけが立つ)（根拠
  `result/summary_wav_lfrq28_nfrq_values.txt`、以下のコマンドで生成）。**傾斜ではなく
  階段で、`0x1f` の 1 点だけが立った。** これで機構が「同じ語を 2 回引いている」側に決まり
  （[§6.2](#62-nfrq-の関与のしかた)）、帯 `0x24`〜`0x2c` に広げる第 2 段は要らなくなった。

  ```bash
  cd test/lfo_noise
  NF=$(python3 -c "print(','.join('%02x' % i for i in range(32)))")

  # LFRQ=0x28 の 1 点 × NFRQ 全 32 値 × AM/PM = 64 本（約 2.5 時間）
  ./capture_lfo.py --wav-dir ./wav_lfrq28_nfrq \
    --lfrq-start 28 --lfrq-end 28 --steps 256 --nfrq "$NF"
  ./capture_lfo.py --wav-dir ./wav_lfrq28_nfrq --check --steps 256   # 短い 0 件を確認
