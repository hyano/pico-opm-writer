...
| `NFRQ` とノイズ語の更新周期の関係 | ノイズ発生器が新しい語を出す周期 = `(32 - NFRQ) / 2` samples | [§3](#3-結論の根拠) `[J]`（`NFRQ=0x00`〜`0x0c` を搬送波 2 種 × AM/PM の 4 系列で実測。実測/予測 0.99〜1.02。PM は `0x0f` まで） |
...
- **`NFRQ > 0x10` の `P_n`**。[§5.4.2](#542-段ごとの-lfo-値から供給周期を測る) で直接測れたのは `NFRQ <= 0x0f` まで。
  `P_lfo` の下限が 8 サンプル（`LFRQ=0xff`）なので、この測定系が探れる `P_n` は
  8〜16 サンプル、つまり `NFRQ` の `0x00`〜`0x10` に限られる。それより速い側は
  「毎段が新しい語」に飽和して速さを区別できない。φM を下げても `P_lfo` と `P_n` の比は
  変わらないので、現在の測定系では到達できない。**`(32 - NFRQ)/2` の上半分は外挿。**
...
- **`0x28` 付近で更新が飛ぶ仕組み**: 上が済んだら、**同じ帯（`LFRQ=0x24`〜`0x2c`）で `NFRQ` を
  ...
  ノイズ発生器の**走る速さ**との噛み合わせは既に落ちている（[§7](#7-不明点未解決事項)。この帯では
  ノイズが 4096 倍以上速い）が、**LFSR の系列そのものの周期**との噛み合わせなら残る。
  `LFRQ=0x28` / `NFRQ=0x1f` では 1 ラッチあたり `65536 × 2 = 131072 = 2^17` シフト進むので、
  LFSR の系列周期が 2^17 前後なら 1 ラッチごとにほぼ同じ状態を引く。これが原因なら
  **山が出るのは `NFRQ=0x1f` だけ**のはずで、帯全体で出るなら別機構と分かる。
  ただしこの説明でも下位ニブル依存（`0x28` が頂点で `0x24`/`0x2c` では出ない）は説明できない
  ので、有力な仮説ではなく**振り分けのための予測**として置く。キャプチャは `--steps 256` で
  9 × 31 × 2 モード。
- **`NFRQ > 0x10` の `P_n` を直接測る**: [§7](#7-不明点未解決事項) のとおり LFO 経由では飽和して届かない。
  ノイズを LFO に通さず**直接出す**（`0x0F` の bit7 = NE でチャネル 8 の 1 オペレータを
  ノイズに置き換える）と、DAC 出力がノイズ列そのものになるので `P_n` を 32 値すべてで
  直接測れる。`opm_seq.txt` とは別のシーケンスと、正弦波を仮定しない解析器が要る。
  これができれば `(32 - NFRQ)/2` の外挿部分が実測に変わる。
