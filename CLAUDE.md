...
| ディレクトリ | 内容 |
| --- | --- |
| [test/lfo_noise/](test/lfo_noise/README.md) | LFO ノイズ波形の調査（`LFRQ` / `NFRQ` の掃引）。掃引・解析とも完了済みで、結論と根拠は README §1〜§3、実測値は `result/`。`wav_w1/`（矩形）だけは**一次データのみで解析は未了**（README §5.7 / §8） |
