...
- **出力側（OPM → Pico）**: SO / φ1 / SH1 / SH2 / /IRQ は 5V デバイスである OPM の**出力**で、
  ...
  **GP26-GP28（I2S の BCK / LRCK / DIN）に 5V デバイスを繋いではいけない。**
  ...
  A2 の個体を使う場合は SO / φ1 / SH1 / SH2 / /IRQ にレベル変換器か分圧を挟むこと。
...
- **逆流防止**: /RD を H に保っていれば OPM は D0-D7 を駆動しないため、5V が Pico の
  ...
  **ただしこれが成り立つのは GPIO を初期化した後だけ。** RP2350 のパッドはリセット直後
  入力 + プルダウン（`PADS_BANK0_GPIO0_RESET` の `PDE` = 1）なので、電源投入から
  `opm_init()` が /CS と /RD を H にするまでの間は OPM から見て /CS = L / /RD = L となり、
  **OPM が D0-D7 を駆動する窓がある**。この窓では Pico 側は入力のままなので駆動は衝突せず、
  5V が乗るのも上記のとおり許容範囲。塞ぎたければ /CS を 5V へ 10kΩ 程度でプルアップする。
...
| シーケンサ（VGM / MDX） | 50µs | この間隔ぶんが `SEQ LAG` に乗る |
...
このとき前の曲は止まったままになる（[§3.14](#314-vgmvgm-再生)）。
...
（[§3.20](#320-ボタンgp21--gp22)）。

| ボタン | 相当するコマンド |
| --- | --- |
| SW1 長押し | `autoplay mode list` + `autoplay start` |
| SW2 長押し | `autoplay mode random` + `autoplay start` |
| SW1 短押し | `autoplay next`（停止中は SW1 長押しと同じ） |
| SW2 短押し | `autoplay prev`（停止中は SW2 長押しと同じ） |
...
`storage host` が誤爆する**ため（SW1 を 0.9 秒押しているところへ SW2 が入ると、
0.1 秒後には両方が押されている状態で 1 秒に達してしまう）。副作用として、
SW1 を 0.9 秒押してから SW2 を足すと SW1 単独の長押しは成立せず、
1.9 秒で同時長押しになる。
...
したときにキャプチャ中だと `ERR wrong state` になり、前の曲は止まったままになる。
