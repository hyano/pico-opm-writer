...
- `clock 4` / `clock 3.58` で手動切り替え（[§3.15](#315-clockクロック切り替え)）
...
| `h` | `h` / `?` | コマンド一覧を表示 |
| `clock` | `clock` / `clock 4` / `clock 3.58` / `clock auto` / `clock fixed` | φM の表示と切り替え（[§3.15](#315-clockクロック切り替え)） |
...
追従してほしくないときは `clock fixed`（[§3.15](#315-clockクロック切り替え)）。
