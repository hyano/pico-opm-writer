...
Raspberry Pi Pico 2 (RP2350) を YM2151 (OPM) に直結して鳴らすためのファームウェア。
ホスト PC から USB シリアルで**レジスタを直接叩く**ことも、内蔵フラッシュに入れた
**VGM / MDX を Pico 単体で演奏する**こともできる。鳴った音は YM3012 (DAC) から PCM として
取り込むので、**USB 経由の録音**と **I2S DAC からのアナログ出力**が外部の機材なしで
同時に得られる。

- **レジスタ書き込み**: USB CDC (仮想 COM ポート) にテキストコマンドを打つだけでよく、
  `screen` や `minicom` のような素のターミナルで足りる（[§3](#3-ホストインタフェース)）。
  バッチ実行やキャプチャの自動化には [ホスト側ツール](#10-ホスト側ツール)（`tools/`）を使う
- **VGM 再生**: `/VGM/` に置いた `.vgm` / `.vgz` を、ホストからシーケンスを流し込まずに
  再生する。ファイルが申告するクロックへ φM を自動で合わせる（[§8](#8-vgm-再生)）
- **MDX 再生**: `/MDX/` に置いた X68000 の MDX を、MXDRV 相当の音源ドライバで
  FM 8ch + ADPCM 8ch 再生する。ADPCM (PDX) はソフトウェアでデコードして FM に混ぜる
  （[§9](#9-mdx-再生)）
- **曲データの受け渡し**: 内蔵フラッシュ 4MiB の一部を FAT にしてあり、USB マスストレージ
  として PC から見える。SD カードも外付けフラッシュも要らない（[§7](#7-ストレージ)）
- **PCM 出力**: YM3012 の出力を 2 本目の USB CDC から PCM で吸い出せる（[§4](#4-pcm-出力)）。
  同じデータを I2S で PCM5102A へ常時流している（[§5](#5-i2s-出力)）。ロジックアナライザや
  外部の録音機材は要らない
- **PC を繋がない運用**: ボタン 3 個で自動連続再生とストレージの切り替えができる
  （[§3.17](#317-autoplay自動再生) / [§3.20](#320-ボタンgp21--gp22)）
...
- **出力側（OPM → Pico）**: SO / φ1 / SH1 / SH2 / /IRQ は 5V デバイスである OPM の**出力**で、
  H レベルは 3.3V を超える。**RP2350 の GPIO 0-25 は 5V トレラント**なので直結でよい
  （[RP2350 データシート](https://datasheets.raspberrypi.com/rp2350/rp2350-datasheet.pdf)
  §14.8.2.1 Table 1426 の `Digital IO (FT)` / §14.9.1 Table 1433 の `VPIN_FT`）。
  成立の条件は次の 2 つで、本構成はどちらも満たしている。

  | 条件 | 本構成での成立 |
  | --- | --- |
  | 対象は **GPIO 0-25 のみ**（`Digital IO (FT)` パッド）。GP26-GP29 は ADC 兼用の `Digital IO / Analogue` で対象外 | 5V を受けるのは GP2-GP9 (D0-D7) / GP16 (/IRQ) / GP17-GP20 (SO・φ1・SH1・SH2) だけで、すべて範囲内 |
  | 5V が掛かっている間 **IOVDD が 3.3V で給電されている**こと。無給電のまま 5V を掛けるとパッドが壊れる | OPM の 5V は Pico の VBUS から取るので、5V があるときは必ず Pico の 3.3V も生きている |

  **ステッピングによる違いは無い。** データシートの FT パッドの規定は版を区別しておらず、
  Appendix C も A4 を "This stepping has no hardware changes." としている。
  A4 の[告知](https://www.raspberrypi.com/news/rp2350-a4-rp2354-and-a-new-hacking-challenge/)
  （2025-07-29）は認定試験を通したという告知であってシリコンの変更ではないので、
  出回っている A2 の Pico 2 でも同じように使える
  （[Raspberry Pi のエンジニアの明言](https://forums.raspberrypi.com/viewtopic.php?p=2330278)）。

  絶対最大定格は GPIO 0-25 が IOVDD = 3.3V のとき 5.5V、IOVDD = 0V のとき 3.63V
  （Table 1433 の `VPIN_FT`）。それ以外のピンは IOVDD + 0.5V。/IRQ は
  [opm.c](src/opm.c) が内部プルアップを張るので、外付け抵抗は要らない。
...
- **バスの衝突防止**: D0-D7 は双方向なので、OPM と Pico が同時に駆動すると衝突する。
  ...
  **読み出し（[`r`](#321-r読み出し)）の実行中以外は /RD を H に固定**している。
  ...
  上の表の条件（GPIO 0-25・IOVDD 給電中）がそのまま効く。
