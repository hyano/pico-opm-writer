...
- **バスの衝突防止**: D0-D7 は双方向なので、OPM と Pico が同時に駆動すると衝突する。
  /RD を H に保っていれば OPM は D0-D7 を駆動しないため、ファームウェアは /RD を常に
  H にしている（読み出しは実装していない）。
...
| 応答 | 発生条件 |
| --- | --- |
| `ERR unknown command` | 未知のコマンド。1 文字のコマンドか `clock` / `storage` / `vgm` / `mdx` / `autoplay` / `help` のいずれでもない場合と、それらのサブコマンドが未知の場合（`clock 5` のような未知のプリセット名も含む） |
...
| シーケンサ（VGM / MDX）と曲送り | 50µs | この間隔ぶんが `SEQ LAG` に乗る。曲送り（autoplay）は同じ周回でシーケンサの後に見るので、更新された再生状態をそのまま読める |
