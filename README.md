...
演奏対象は **FM 8ch だけ**で、ADPCM パートは本機にハードウェアが無いので鳴らない。
詳しくは [§9](#9-mdx-再生) を参照。
...
PDX（ADPCM の波形集）を要求する曲では、その名前と注意書きが出る。FM パートは
そのまま正常に鳴る。
...
# ch      : 9  voices 20
# pdx     : THEXDER
# hint    : ADPCM パートは発音しない（FM パートだけ鳴る）
OK
```
...
**演奏対象は FM 8ch だけ。** MDX の MML コマンドは全部解釈する（音色・音量・パン・
ゲートタイム・レガート・リピート・ループ・ディチューン・ポルタメント・ソフト LFO
（音程 / 音量）・OPM のハードウェア LFO・同期・フェードアウト・キーオン遅延）。

**ADPCM パートは鳴らない。** X68000 の ADPCM は MSM6258 という別のチップで、
本機には載っていない。PDX（波形集）を要求する曲でもエラーにはならず、FM パートだけが
正常に鳴る。ただし ADPCM チャンネルも**シーケンサとしては FM と同じように回している**。
同期コマンド（`0xEF` / `0xEE`）が ADPCM 側から FM 側へ飛んでくる曲があり、
止めてしまうと FM が永久に待ち続けるため。
...
[§8.4](#84-既存機能との共存)）。MDX はファイルを丸ごと RAM に載せているので
アンマウントされても読めるが、フラッシュの消去中は数十 ms 止まるので拒否する。
...
- **MDX の ADPCM パート（PCM8 / 8ch）。** 本機に MSM6258 は無いので、鳴らすなら
  8ch ぶんの ADPCM をソフトでデコードし、YM3012 から取り込んだ PCM に加算して
  I2S / CDC #1 へ流すことになる。混ぜる場所は `i2s.c` が YM3012 の PCM を
  I2S リングへ書いた直後（`capture.c` にも同じ継ぎ目がある）。
  制約は「出力レートが φM/64 固定なので ADPCM の 3.9 / 5.2 / 7.8 / 15.6kHz からは
  整数比にならず補間が要る」「PCM8 はチャンネルごとにレートが違いうる」
  「8ch を 32bit で足してから 1 回だけ飽和させる（各チャンネルで飽和させると歪む）」
  「PDX は数百 KB あるので MDX 本体のように丸ごと RAM には載せられない」
