...
DAC 出力のキャプチャには [ホスト側ツール](#9-ホスト側ツール)（`tools/`）を使う。
...
| `ERR bad file` | VGM として読めない（マジック不正 / `.vgz` / ヘッダが壊れている） |
...
1 文字のコマンドは大小どちらでもよい。`storage` / `vgm` とそのサブコマンドも同様。
`vgm play` のファイル名だけは行の残り全部を 1 引数として受けるので、空白を含む名前も
そのまま書ける（`vgm play BAD NAME.VGM`）。
...
| `VGM` | VGM の再生状態（`STOPPED` / `PLAYING` / `ERROR`）と再生中のファイル名 |
| `VGM POS` | 発行済みのサンプル位置 / ヘッダの総サンプル数と、ループした回数 |
| `VGM LAG` | VGM スケジューラが予定時刻から遅れた最大時間と、時計を張り直した回数（[§3.14](#314-vgmvgm-再生)） |
...
他の音源のコマンドは読み飛ばす。詳しくは [§8](#8-vgm-再生) を参照。

| コマンド | 説明 |
| --- | --- |
| `vgm list` | `/VGM/` の `.vgm` を名前順（大小無視）に並べる |
...
PC にリムーバブルディスク `OPMVGM` が現れるので、`/VGM/` へ `.vgm` ファイルを置く。
終わったら **PC 側で取り出し（eject）する**。それだけで Pico 側へ所有権が戻り、
FatFs が自動でマウントし直される。
...
| 処理するコマンド | `0x54`（YM2151 書き込み） / `0x61` `0x62` `0x63` `0x70`-`0x7F`（wait） / `0x80`-`0x8F`（DAC。書き込みは飛ばし wait だけ効かせる） / `0x66`（終端） / `0x67`（データブロックをシークで飛ばす） |
...
| 非対応 | `.vgz`（gzip 圧縮）。`gunzip` してから転送すること。GD3 タグの表示も未実装 |
...
### 8.3 φM と VGM のクロックが違うとき

既定の φM は 4.000000MHz だが、YM2151 の VGM は 3.579545MHz 指定のものが多い。
食い違っていても再生はするが、`vgm play` のときに警告を出す。
...
3.579545MHz のコレクションを正しい音程で鳴らしたいときは、φM のプリセットを
切り替えてビルドする（[§2](#2-クロック設定-φm)）。

```bash
cmake -S . -B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DPICO_BOARD=pico2 -DOPM_CLOCK_MODE=1
```

このときキャプチャと I2S のサンプリングレートも φM/64 = 55930Hz に変わる。
