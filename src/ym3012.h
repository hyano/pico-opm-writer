...
/*
 * 無音とみなす振幅。YM3012 の出力は消音時に 0 付近まで落ちるので、量子化の
 * 端数だけを拾わない程度に小さく取ってある（32768 に対して -66dB 相当）。
 */
#define YM3012_QUIET_LEVEL 16

/*
 * 最後に「無音でない」フレームを見た時点の総フレーム数。
 *
 * ym3012_write_total() との差がそのまま「無音が続いているフレーム数」になる。
 * 更新は ym3012_ring_poll() が取り込みのついでに行うので、I2S もキャプチャも
 * 動いていない構成でも進む。
 */
uint64_t ym3012_last_loud_total(void);

/*
 * 長く止まったあと（フラッシュの消去・書き込みなど）に呼ぶ。書き込み位置を取り込み直し、
 * 既定カーソルを現在の位置へ合わせる。
 *
 * 差分は ym3012_ring_poll() と同じく必ず積む。s_write_total ≡ 書き込み添字
 * (mod YM3012_RING_FRAMES) という位相が、「フレーム番号 f の中身は s_ring[f & 4095]」
 * という前提を支えているので、ここを飛ばすと止まっていた分だけ音が古くなる。
 * 一周 65.5ms を超えて止まった場合は総フレーム数が一周単位で少なく出るが、それは
 * 統計（FRAMES / RATE）だけの誤差で、位相は必ず合う。
 */
...
/*
 * ゲインを 1.0 に戻す境界を置く。冪等。
 *
 * 戻り始めるのは「**いま取り込んでいるフレーム** + delay_frames」からで、そこより前は
 * 落とし切ったままにする。即座に戻すと、消費者がまだ読んでいないフェード済みの区間が
 * 全音量で出てしまい、曲の切り替わりで音が跳ねる。基準は ym3012_ring_poll() を呼んで
 * 引き直してから置くので、呼び出し側が音声チェーン（500us 間隔）を待つ必要は無い。
 *
 * delay_frames はチップのリリースを隠すための猶予。キーオフの直後に呼んでも、チップの
 * 音は RR=15 でも数 ms 残る（実測: 最悪ケースで 0 まで約 5.5ms）。ここを 0 にすると
 * その区間が全音量で出る。
 *
 * ramp_frames は境界から 1.0 まで戻すのにかける長さ。0 なら境界で一気に戻す。猶予の
 * 見積もりが外れて音が残っていても、ランプがあれば段差にならない。カーブは
 * ym3012_fade_start() の q^2 を時間で反転したもの。
 *
 * **境界は前へしか動かない。** 1 回のフェードのあいだ単調減少し、後ろへ動かす要求は
 * 黙って無視する（一度解除した区間を無音に塗り直さないため）。次の
 * ym3012_fade_start() で解除される。
 */
void ym3012_fade_release(uint32_t delay_frames, uint32_t ramp_frames);

/* ym3012_fade_release(0, 0) と同じ。猶予もランプも要らない呼び出し用。 */
