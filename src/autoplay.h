 ...
 *
 * **曲の終わり方そのものは songend.c が持つ。** ループ回数での打ち切りも、
 * フェードアウトも、終わったあとの余韻を待つのも向こうの仕事で、こちらは
 * songend_is_active() が落ちるのを待って次の曲へ送るだけ。曲間の無音は
 * 余韻が消えてから数え始まる。
 *
 * loop / fade はここにも設定があり、既定値が手動再生と違う（自動再生は 2 周で
 * フェード、手動は無限）。トラックを始めるたびに songend_arm() でこちらの値を
 * 渡して上書きする。
 *
...
{
    AUTOPLAY_STOPPED = 0,
    AUTOPLAY_PLAYING, /* 曲を鳴らしている（songend の FADING / RINGOUT も含む） */
    AUTOPLAY_GAP,     /* 曲間の無音 */
} autoplay_state_t;
