 ...
 * ---- 参照実装との対応 --------------------------------------------------
 *
 * 解釈は X68k MXDRV music driver version 2.06+17 Rel.X5-S / for Win32
 * [MXDRVg] V2.00b に合わせてある。一部の機能だけ MXDRV 2.06+16 Rel.3+25 に従う。
 *
 ...
 * それ以外の L00xxxx は **2.06+17 Rel.X5-S の逆アセンブル**の番地で、番地は対応しない。
 *
 ...
 * シャドウに控える / レジスタ 0x1B だけ別に控える」をやっている。本機は BUSY 待ちの
 * 代わりに opm_write() の固定ウェイト（アドレス後 5us / データ後 25us）を使う。
 * ステータス自体は opm_read() で読めるが、書き込み経路では見ない。ここだけが
 * 参照実装との相違点。シャドウは 0x1B の read-modify-write に要るのでそのまま持つ。
 *
 * ---- ADPCM (PCM8) ------------------------------------------------------
 *
 * 本機に MSM6258 は無いので、ADPCM パートは pcm8.c が PDX の波形をソフトウェアで
 * デコードして FM の PCM へ加算する。ここが持つのはシーケンサ側だけで、発音の
 * 操作は snd_* の出力バックエンドから pcm8.h の API を呼ぶ形にまとめてある
 * （ほかに 0xE7 0x02 の PCM8 コマンドと、mdx_play() / mdx_stop() の PDX 開閉が呼ぶ）。
 *
 * 叩き先は 0xE8（PCM8 モード宣言）を読んだかどうかで変わる（s_pcm8_mode =
 * 参照実装の PCM8_FLG）。PCM8 モードは 8ch で音量・バンク・24bit 長が使え、
 * 非 PCM8 モードは IOCS 相当なので ch0 の 1 音だけ・原音量固定・バンク 0 固定に
 * なる。両者の違いの一覧は docs/pico-opm-writer.md §10.7 の表。
 *
 * PDX が開けないときや PCM8_ENABLED=0 のビルドでも、ADPCM チャンネルはシーケンサ
 * としては FM と同じように回す（同期コマンドが ADPCM 側から飛んでくる曲があるため）。
 */
#include "mdx.h"
...
/*
 * 音を出す操作はすべてここを通す。is_adpcm() で振り分けて、FM は opm_put() で
 * OPM のレジスタへ、ADPCM は pcm8.h の API へ出す。
 */
 ...
 *
 * opm_put() を経由しないぶん、書いた値をシャドウへ自分で反映する。opm_put() で
 * 書いていたときと同じ値になるようにしておかないと、次の tick が「もう同じ値が
 * 入っている」と判断して書き直しを省くことがある。
 */
static void key_off_all(void)
{
    opm_key_off_all();
    for (uint32_t reg = 0xe0u; reg <= 0xffu; reg++)
    {
        s_opm[reg] = 0x0fu;
    }
    s_opm[0x08u] = 7u; /* 最後に書いたチャンネル番号 */
    pcm8_abort_all();
}
