...
LED → 統計、をひと回しする。**ボタンの消化**とコマンド 1 文字読みは `main()` の側。
...
| BUSY 待ち | `OPM_BUSY_CYCLES`（72 φM）から実 φM で実行時に算出するので、切り替えのたびに計算し直される（[§3.1](#31-タイミング定数)） |
...
`sys_clk` に依存して実行時に算出している値（[opm.c](../src/opm.c) の `s_setup_cycles` /
`s_data_wait_cycles` と φM の分周比、[i2s.c](../src/i2s.c) の `s_div256`）は、
切り替えのたびに計算し直す。
...
[データサイクル]
  ...
  データ確定を待つ (t_SETUP)
  /WR = L → t_WR 保持 → /WR = H
  BUSY 待機     ← OPM の内部処理（BUSY）が終わるまで。72 φM サイクル
```
...
後も、データサイクル後は **BUSY が明けるのに要する時間だけ待つ**（`OPM_BUSY_CYCLES`）
ままにしてある。BUSY 長が完全に一定であることが実測で確定しているので、
ポーリングには副作用しか無い（[§3.1](#31-タイミング定数)）。
...
| `OPM_BUSY_CYCLES` | 72 φM | データ書き込み後の BUSY 待ち。**µs ではなく φM サイクルで持ち、実 φM から実行時に算出する**（4MHz で 18.0µs / 3.579545MHz で 20.1µs）。実測の 67.1 サイクルに約 7% の余裕を足した値（[test/opm_busy/](../test/opm_busy/README.md)） |
...
**`OPM_BUSY_CYCLES` 以外はデータシートの最小値から詰めた値ではなく、余裕を大きく取った
仮置きである。** この値で [test/](../test/) の全キャプチャ（数千条件）が取れているので
「短すぎはしない」ことは分かっているが、どこまで短縮できるかは測っていない（未確認）。

`OPM_BUSY_CYCLES` だけは実測に基づく。BUSY はデータサイクルから **67.1 ± 0.2 φM
サイクル**で落ち、**レジスタアドレスにも書き込む値にもチップの発音状態にも依存しない**
（[test/opm_busy/](../test/opm_busy/README.md)）。したがって

- **単位は µs ではなく φM サイクル。** `s_setup_cycles` と同じく実行時に算出するので、
  φM プリセットを切り替えても定数の書き換えが要らない。算出は
  `opm_clock_compute()` が目標値ではなく**実 φM**（`s_clock_hz_actual`）で行う。
  クロック切り替えの途中で使う切り上げ分周比では実 φM が目標を下回るため、
  目標で割ると待ちが足りなくなる
- **BUSY のポーリングには置き換えない。** 削減幅は固定待ちと同じ 7µs で、
  副作用（チップ不在時の無限ループ、アサート遅延 0.33µs の余裕、D0-D7 が 5V を
  受ける時間の増加）だけが増える（同 README の「案 C を採る理由が無い」）
- **待ちの繰り延べもしない。** 平均は下がるが最悪は下がらず（`opm_clear()` の密ループには
  重ねる仕事が無い）、φM の張り替え前と `opm_reset()` の前に未消化の待ちを消化する
  制約が新たに増える

1 レジスタ書き込みは約 25µs（最大 4 万回/秒）。実測値は `s` の `OPMW` 行に出る
（[README §3.11.1](../README.md#3111-サービスの呼び出し間隔)）。
