...
MDX の発行 → ストレージの書き出し → LED → 統計 → コマンド 1 文字読み、を回すだけ。
...
レジスタへの書き込み内容と順序は **MXDRV 2.06+17** に合わせてある。
...
### 10.6 出力バックエンドと ADPCM の拡張点

音を出す操作は `snd_voice()` / `snd_pan()` / `snd_volume()` / `snd_pitch()` /
`snd_key_on()` / `snd_key_off()` の 6 本に集約してあり、**ADPCM チャンネルではこの中で
何もしない**。`if (ch < 8)` を処理のあちこちに撒かないための構造で、将来 PCM8 (8ch) の
発音を足すときはこの 6 本に手を入れるだけで済む。
...
9-15ch は `0xE8`（PCM8 モード宣言）が実行されてから回り始める。ヘッダが持つ
PDX ファイル名も捨てずに保持し、`mdx_pdx_name()` から読める。

将来 ADPCM を鳴らすなら、8ch ぶんをソフトでデコードして YM3012 から取り込んだ PCM に
加算することになる。**混ぜる場所は [i2s.c](../i2s.c) が `ym3012_reader_read_pcm()` で
I2S リングへ L/R 交互の `int16_t` を書いた直後**（[capture.c](../capture.c) にも同じ
継ぎ目がある）。制約は [§5.2](#52-サンプリングレートのロック) のとおり出力レートが
φM/64 に固定されていること、PCM8 はチャンネルごとにレートが違いうること、
8ch を 32bit で足してから 1 回だけ飽和させる必要があること
（チャンネルごとに 16bit へ飽和させると歪む）、そして PDX は数百 KiB あるので
MDX 本体のように丸ごと RAM には載せられないこと。

### 10.7 既存機能との共存

VGM と MDX は相互排他で、どちらかが再生中ならもう一方の `play` は `wrong state`。
...
読めるが、フラッシュの消去中は数十 ms 止まるので `storage host` は再生中拒否する。
