...
| [button.h](../button.h) / [button.c](../button.c) | GP21 / GP22 の取り込み。デバウンス、短押し / 長押しの状態機械、イベントのメールボックス（[§12](#12-ボタン入力)） |
...
唯一の点**。イベントは深さ 1 のメールボックスで渡す（[§12](#12-ボタン入力)）。
...
ボタン（[§12](#12-ボタン入力)）。取り込みと消化が離れているので、
...
LED の一時表示と、起動時のモード提示（[§12.5](#125-led-の一時表示と起動待ち)）。
...
ステータスを読んで bit7 が下りるのを待つが、本機は `opm_write()` の固定ウェイト
（合計約 32µs、[§3.1](#31-タイミング定数)）で代える。ステータス自体は `opm_read()` で
読めるが、書き込み経路では見ない（[§3](#3-opm-バスシーケンス)）。X68000 実機より
遅い方向にしかならない。
