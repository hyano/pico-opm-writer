...
リングを無音で埋めて位置を張り直す。なお `ym3012_ring_resync()` は総フレーム数を
進めずに基準点だけ張り直すので、通常の復帰ではこの経路には入らない。
...
**解除するのは停止（キーオフ）の直後**で、`stop_playback()` が 2 つの `*_stop()` の
後ろで呼ぶ。順序が逆だと、まだ鳴っているチップの音が全音量で出てしまう。停止側は
`key_off_all()` が全スロットの RR を 15 にしてから落とすので、チップが
黙るまでは 1ms もかからず、戻す対象は消音済みの区間だけになる。曲送りの経路のうち
`autoplay next` / `prev` は `GAP` を挟まないので、ここに置くことで `GAP` 経由と
挙動がそろう。
