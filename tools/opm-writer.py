...
この 2 つ以外の拡張子は受け付けない。WAV に書くサンプリングレートは φM/64。

- `!capture <ms>` は `--phim` から計算する。**実機の φM と合わせること**
  （ずれると時間軸のずれた WAV が黙って出来上がる）
- `!capture-song` は演奏を始めたあとに `p` を投げて実機のレートを読む。曲が φM を
  切り替えていることがあるため（VGM の自動追従）。`--phim` は読めなかったときの
  フォールバックにしか使わない
