...
/*
 * 曲は VGM_DIR / MDX_DIR からの相対パス（例 `KONAMI/GRADIUS.VGM`）で持つ。'\0' 区切りで
 * プールへ詰め、s_offs にプールへのオフセットを `vgm list` / `mdx list` と同じ深さ優先の
 * 順で並べる。曲の種別は配列を持たず、s_offs 上の位置が s_vgm_count 未満かどうかで
