 ...
 *
 * ADPCM (MSM6258 / PDX) パートは本機にハードウェアが無いので発音しない。ただし
 * シーケンサとしては FM と同じように回す。同期コマンド (0xEF/0xEE) が ADPCM 側
 * から飛んでくる曲があるためと、将来 PCM8 (8ch) をソフトデコードして混ぜられる
 * ようにしておくため。発音の有無はチャンネルごとの出力バックエンドで切り替える。
 *
 ...
 * 丸ごと載せてしまう方が単純で速い。PDX はこの上限とは別扱い（将来）。
 */
