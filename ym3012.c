...
{
    /*
     * **差分は必ず積む。** s_write_total とリング内の位置は
     *   s_write_total ≡ DMA の書き込み添字 (mod YM3012_RING_FRAMES)
     * で結ばれていて、これが「フレーム番号 f の中身は s_ring[f & 4095] にある」という、
     * 読み出し・フェードのゲイン・PCM8 のミックスリングが共通で頼っている前提そのもの。
     * ここで基準点だけを張り直すと止まっていた分だけ位相がずれ、以後フレーム番号が
     * 指すより古い音が返るようになる。ずれは blackout のたびに積み上がる。
     *
     * 65.5ms を超えて止まると進んだ分がリング一周単位で切り捨てられ、総フレーム数は
     * その分だけ少なく出る。ただしそれは FRAMES / RATE の統計だけの誤差で、位相は
     * どれだけ止まっても必ず合う。フラッシュの書き出しは 1 セクタごとにここへ来るので
     * 実測でも 42ms 止まりで、一周には届かない。
     */
    ym3012_ring_poll();
